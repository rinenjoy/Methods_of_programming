"""
🔴 CLAWS-ACTUATORS — Приводы клешней (недоверенные сервоприводы).
HTTP-запрос к wall-e для физической активации захвата.
"""
import os
import json
import threading
...
def handle_event(id, details_str):
    ...
    if operation == "actuate":
        try:
            r = httpx.post(f"{WALL_E_URL}/claws", json=data, timeout=5.0)
            print(f"[{MODULE_NAME}] wall-e claws: {r.json()}")
        except Exception as e:
            print(f"[error] claws: {e}")
