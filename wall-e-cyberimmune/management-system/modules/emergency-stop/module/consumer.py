"""
🟢 EMERGENCY-STOP — Экстренная остановка (полностью доверенный).
Простая логика: получил команду stop → передал команду halt drive-actuators.
"""
import os
import json
import threading
...
def handle_event(id, details_str):
    ...
    if operation == "emergency_stop":
        print(f"[{MODULE_NAME}] HALT: {data.get('reason')}")
        send_to("drive-actuators", "halt", data)
